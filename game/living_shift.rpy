...
init 12 python:
        ...
        "packing_scan": {
            ...
            "hotspots": [
                ...
                },
                {
                    ...
                    "detail": "Новичок оставила её там, где пыталась одна остановить хвост.",
                    "x": 1500,
                    "y": 690,
                    "route": "newbie",
                    "stat": ("ps_humanity", 1),
    ...
    ps_live_messages = [
        ...
        },
        {
            ...
            "incoming": [
                "СМЕНА СЛЫШИТ ТЕБЯ.",
                "СОБЕРИ ПЯТЬ СЛЕДОВ. НЕ ДОВЕРЯЙ ВРЕМЕНИ НА КАМЕРАХ.",
            ],
            "replies": [
                {
                    "id": "who",
...
label ps_storm_interference:
    ...
    n "Среди аварийных команд появляется ещё один ритм."
    n "Он не идёт из рации."
    n "Телефон сам открывает канал V-13."
    ...
    if ps_storm_decoder_completed:
        ...
        n "Последний тон совпадает."
        n "Помехи складываются в человеческий голос."
        n "«Сектор просыпается не после аварии»."
        n "«Авария начинается, когда сектор просыпается»."
    else:
        $ ps_burnout += 1
        n "Ты разрываешь соединение."
        n "Но последний импульс продолжает вибрировать в корпусе телефона."
...
label ps_route_afterword:
    ...
    if ps_afterword_route == "newbie":
        ...
        p "Что?"
        newb "Теперь, когда система пишет красным, я сначала читаю."
        newb "А уже потом решаю, виновата ли я вообще."
        p "Хорошая привычка."
        ...
        p "Добровольно?"
        vet "Не порть исторический момент."
        vet "Смена переживёт один мой выходной."
        p "А ты?"
        ...
        p "Кого?"
        mem "Всех."
        mem "Сегодня никто не исчез между отчётом и проходной."
        ...
        p "А ты?"
        sv "Что неисправный подъёмник создаёт риски быстрее."
        sv "Первый раз написал это без чужого имени."

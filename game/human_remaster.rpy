...
label ps_human_shift_scene(day):
    ...
    if day == 2:
        ...
        n "Перерыв уже закончился, но никто не встаёт первым. Лера медленно сворачивает пустой пакет из-под печенья, Виктор греет ладони о давно остывшую кружку, а Макс смотрит на часы, ожидая официального разрешения ещё немного побыть людьми."
    ...
    if day == 5:
        ...
        menu:
            ...
            "Перераспределить людей с подходящими перчатками[ps_choice_hint('решить проблему без опасной спешки')]":
                ...
                n "Семь минут всё равно теряются, но впервые никто не пытается выиграть их за счёт чужого дискомфорта."
        return
...
label ps_last_checkpoint_scene:
    ...
    menu:
        ...
        "Попрощаться здесь, не обесценивая неделю[ps_choice_hint('дать каждому право уйти своим путём')]":
            ...
            p "Дальше каждый своей дорогой. Эту неделю всё равно никто из нас не забудет."

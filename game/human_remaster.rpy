...
label ps_human_shift_scene(day):
    ...
    if day == 2:
        ...
        menu:
            "Спросить, кем они бывают после смены[ps_choice_hint('узнать людей вне склада')]":
                $ ps_record_human_memory("разговор вне склада", "Ты впервые спросил команду, какой у каждого начинается жизнь за проходной.", 2)
                $ ps_humanity += 1
                $ ps_team_unity += 1
    ...
    if day == 4:
        ...
        $ ps_unlock_cg("team_break_cinematic", True)
    ...
    if day == 7:
        ...
        menu:
            "Сфотографировать табель для команды[ps_choice_hint('сохранить неофициальную память')]":
                $ ps_record_human_memory("человеческий табель", "Ты сохранил табель, где людей описывает забота, а производительность уходит на второй план.", 2)
                $ ps_humanity += 1
                $ ps_team_unity += 1

...
label ps_trace_d2_break:
    ...
    n "До запуска линии остаётся девять минут."
    n "Новичок сидит над сложенным вчетверо листком."

    newb "Я записываю ошибки."
    newb "Не свои. Вообще все, которые вижу."
    ...
    menu:
        "Помочь превратить записи в понятный чек-лист":
            ...
            p "Давай разделим: экран, ячейка, подтверждение."
            p "И отдельно — что делала система."
    ...
    n "Она отрывает половину листка и протягивает тебе."
    n "На обороте — время вчерашней передачи участка."
...
label ps_trace_d2_mezzanine:
    ...
    n "Ветеран проверяет ограждение у грузового подъёмника."
    n "На металлической панели выцарапано: LIFT-09."

    vet "Запомни."
    vet "Если эта лампа мигает дважды — платформа считает вес неправильно."
    ...
    menu:
        "Спросить, сколько раз это уже происходило":
            ...
            $ ps_evidence += 1
            $ ps_integrity += 1
            $ ps_record_consequence("Ветеран рассказал тебе о старой неисправности подъёмника.")
        ...
        "Проверить аварийную кнопку рядом":
            ...
            n "Кнопка закрыта прозрачной крышкой."
            n "Крышка открывается туже, чем должна."

            vet "Хорошо."
            vet "В опасный момент руки должны помнить быстрее головы."
...
label ps_trace_d2_packing:
    ...
    n "На упаковке копится очередь нестандартных товаров."
    n "Супервайзер один переставляет приоритеты на терминале."

    sv "Раз пришёл — выбирай."
    sv "Спасти время или освободить безопасный проход."
    ...
    menu:
        ...
        "Перенаправить мелкие заказы и разгрузить очередь":
            ...
            $ ps_efficiency += 2
            $ ps_supervisor_respect += 1
            $ ps_record_consequence("Ты помог Супервайзеру разгрузить очередь без остановки участка.")
...
label ps_trace_d2_control:
    ...
    n "Диспетчерская пуста."
    n "Один монитор оставлен на журнале передачи смен."
    n "В 02:14 участок отмечен жёлтым."
    ...
    menu:
        "Сохранить номер передачи и время":
            ...
            n "Ты фотографируешь экран."
            n "На снимке нет ответа."
            n "Зато теперь существует вопрос."

        "Проверить, кто подтвердил передачу":
            $ ps_add_route("supervisor", 2)
            $ ps_supervisor_respect += 1
            $ ps_evidence += 1
            $ ps_record_consequence("В журнале передачи ты нашёл подпись Супервайзера.")

            n "Подпись принадлежит Супервайзеру."
            n "Рядом серым: «Изменено после закрытия смены»."
...
label ps_trace_d2_dock:
    ...
    n "Под колесом пустой тележки лежит сорванная пломба."
    n "На ней фиолетовый знак: круг, перечёркнутый тремя линиями."
    ...
    menu:
        "Сфотографировать пломбу":
            ...
            $ ps_humor += 1
            $ ps_evidence += 1
            $ ps_record_consequence("На доке вы с Шутником нашли пломбу несуществующего сектора.")

            p "Теперь хотя бы есть фотография."
            mem "Отлично."
            mem "Если исчезнем — у полиции будет очень атмосферная улика."
...
label ps_trace_d3_break:
    ...
    n "Шутник сидит перед автоматом с кофе."
    n "Стакан пуст. Монеты всё ещё лежат в ладони."
    ...
    menu:
        "Сесть рядом и не торопить":
            ...
            $ ps_humanity += 1
            $ ps_burnout -= 1
            $ ps_record_consequence("Ты позволил Шутнику несколько минут не поддерживать чужое настроение.")

            n "Вы молчите."
            n "Впервые рядом с ним тишина не кажется паузой перед шуткой."
        ...
        "Купить два кофе и перевести всё в шутку":
            ...
            $ ps_humor += 2
            $ ps_team_unity += 1
            $ ps_record_consequence("Короткий перерыв вернул Шутнику голос перед тяжёлой сменой.")
...
label ps_trace_d3_mezzanine:
    ...
    n "На корпусе подъёмника свежая наклейка закрывает старую маркировку."
    n "Ветеран поддевает угол перчаткой."

    vet "Вот."
    vet "LIFT-09. Проверка просрочена."
    ...
    menu:
        "Снять обе маркировки одним кадром":
            ...
            $ ps_evidence += 2
            $ ps_integrity += 1
            $ ps_record_consequence("Вы с Ветераном сохранили настоящую маркировку подъёмника.")

            p "Старая дата и новая наклейка."
            vet "Так даже специалист из офиса поймёт."

        "Попросить Ветерана больше не пользоваться подъёмником":
            $ ps_add_route("veteran", 2)
            $ ps_humanity += 1
            $ ps_team_unity += 1
            $ ps_record_consequence("Ты попросил Ветерана не проверять неисправный подъёмник собственным телом.")
...
label ps_trace_d3_packing:
    ...
    n "Новичок держит коробку с двумя разными этикетками."

    newb "ТСД говорит — электроника."
    newb "На старой наклейке — хрупкое."
    ...
    menu:
        "Остановить операцию и проверить карточку товара":
            ...
            p "Не подтверждай."
            p "Если экран спорит с товаром, сначала верим глазам."
...
label ps_trace_d3_control:
    ...
    n "Супервайзер закрывает таблицу рейтинга, когда ты входишь."

    p "Моё место — секрет?"
    sv "Нет. Способ расчёта — почти."

    n "Он показывает вес показателей."
    n "Скорость учитывается сразу."
    n "Помощь другому участку — только после ручного подтверждения."
    ...
    menu:
        "Попросить учитывать остановки по безопасности":
            ...
            $ ps_integrity += 1
            $ ps_record_consequence("Супервайзер добавил безопасные остановки в ручной отчёт смены.")
        ...
        "Разобраться, как честно поднять эффективность":
            ...
            sv "Убирай повторные движения."
            sv "Не убирай проверки."
            p "Звучит очевидно."
...
label ps_trace_d3_dock:
    ...
    n "За воротами идёт мелкий дождь."
    n "Шутник считает машины, которые уезжают к городу."
    ...
    menu:
        ...
        "Предложить собственный рейтинг смены":
            ...
            $ ps_humor += 2
            $ ps_team_unity += 1
            $ ps_record_consequence("Вы с Шутником придумали рейтинг вещей, которые система не умеет считать.")
...
label ps_trace_d4_break:
    ...
    menu:
        "Разобрать каждую строку вместе":
            ...
            $ ps_newbie_trust += 2
            $ ps_integrity += 1
            $ ps_record_consequence("Новичок не подписала готовое признание в чужой ошибке.")

            p "Пиши только то, что видела сама."
            p "Не их формулировку."
...
label ps_trace_d4_mezzanine:
    ...
    n "Ветеран раскладывает распечатки по времени."
    n "Семь операций принадлежат Новичку."
    n "Остальные сорок — аккаунту человека, которого сегодня нет."

    vet "Учёт задвоился."
    vet "Но виноватым назначат того, кто рядом."
    ...
    menu:
        "Помочь восстановить цепочку":
            ...
            $ ps_evidence += 2
            $ ps_team_unity += 1
            $ ps_record_consequence("Вы с Ветераном восстановили разорванную цепочку операций.")

            p "Сначала буфер, потом повторная привязка."
            vet "И сорок единиц возвращаются к старому аккаунту."

        "Попросить Ветерана выступить свидетелем":
            $ ps_add_route("veteran", 1)
            $ ps_integrity += 1
            $ ps_humanity += 1
            $ ps_record_consequence("Ветеран согласился подтвердить сбой не только в разговоре.")
...
label ps_trace_d4_packing:
    ...
    n "Под упаковочным столом находится коробка с остатками старых этикеток."
    n "Одна из них содержит номер пропавшей партии."
    n "Сорок семь единиц ушли в буфер во время общего сбоя."
    ...
    menu:
        "Снять полную последовательность кодов":
            ...
            n "Номера складываются в простую цепочку."
            n "Не кража."
            n "Не невнимательность."
...
label ps_trace_d4_control:
    ...
    n "Камеры показывают склад с задержкой в несколько секунд."
    n "На канале V-13 изображение внезапно становится фиолетовым."

    if not persistent.ps_reduce_flashes:
        scene bg control_room
        with hpunch

    n "На один кадр появляется коридор, которого нет на плане."
    n "Подпись канала: «СЕКТОР V // ШТОРМОВОЙ ДОПУСК»."
    n "Потом экран возвращается к обычной ленте."
    ...
    menu:
        ...
        "Позвать Шутника как свидетеля":
            $ ps_add_route("joker", 2)
            $ ps_humor += 1
            $ ps_team_unity += 1
            $ ps_record_consequence("Вы с Шутником вдвоём увидели коридор, которого нет на плане.")
...
label ps_trace_d4_dock:
    ...
    n "Супервайзер стоит у закрытых ворот с выключенной рацией."
    ...
    menu:
        "Спросить, сколько раз он это подписывал":
            ...
            $ ps_supervisor_respect += 2
            $ ps_integrity += 1
            $ ps_record_consequence("Супервайзер признал, что раньше закрывал похожие инциденты.")
        ...
        "Потребовать исправить документ Новичка":
            ...
            $ ps_supervisor_respect += 1
            $ ps_humanity += 1
            $ ps_record_consequence("Ты заставил Супервайзера убрать готовое признание из дела Новичка.")
...
label ps_trace_d5_break:
    ...
    n "Ветеран затягивает бинт на запястье зубами."
    ...
    menu:
        "Настоять, чтобы он снялся с тяжёлых операций":
            ...
            $ ps_team_unity += 1
            $ ps_veteran_safe = True
            $ ps_record_consequence("Ветеран согласился не доказывать опыт через боль.")
        ...
        "Попросить показать, как правильно страховать груз":
            ...
            $ ps_endurance += 1
            $ ps_efficiency += 1
            $ ps_record_consequence("Ветеран передал тебе приём безопасной страховки груза.")

            vet "Не держишь вес."
            vet "Держишь направление, в котором он не должен упасть."
...
label ps_trace_d5_mezzanine:
    ...
    n "За щитом управления подъёмником застряла сложенная заявка."
    n "Статус: «Ожидает остановки участка»."
    n "Ниже новая строка: «Отменено в связи с производственной необходимостью»."
    ...
    menu:
        "Сохранить обе версии заявки":
            ...
            n "Заявка создана за девять дней до твоей первой смены."
            n "Неисправность не была неожиданностью."

        "Передать заявку Супервайзеру при свидетелях":
            $ ps_add_route("supervisor", 2)
            $ ps_supervisor_respect += 1
            $ ps_team_unity += 1
...
label ps_trace_d5_packing:
    ...
    n "Поток ускорился, и Новичок начинает повторять каждое действие вслух."
    ...
    menu:
        "Встать рядом и принять часть очереди":
            ...
            $ ps_team_unity += 1
            $ ps_humanity += 1
            $ ps_record_consequence("Во время перегруза Новичок не осталась одна со своей очередью.")
        ...
        "Помочь ей выстроить приоритеты":
            ...
            $ ps_newbie_trust += 1
            $ ps_efficiency += 2
            $ ps_record_consequence("Новичок удержала перегруз, используя собственный порядок действий.")

            p "Сначала тяжёлое в стоп."
            p "Потом хрупкое. Остальное ждёт."
...
label ps_trace_d5_control:
    ...
    n "На центральном экране две цифры."
    n "План: 91%%."
    n "Риск перегруза: высокий."
    ...
    menu:
        "Предложить частичную остановку опасной линии":
            ...
            $ ps_efficiency += 1
            $ ps_integrity += 1
            $ ps_record_consequence("Вы с Супервайзером нашли схему частичной безопасной остановки.")

            p "Останавливаем подъёмник, не весь участок."
            p "Мелкое уходит через левую линию."
        ...
        "Попросить его честно объявить риск смене":
            ...
            $ ps_team_unity += 2
            $ ps_humanity += 1
            $ ps_record_consequence("Супервайзер впервые объявил риск перегруза всей смене.")
...
label ps_trace_d5_dock:
    ...
    n "Шутник рисует пальцем схему линий на пыльной двери."
    ...
    menu:
        "Придумать короткие голосовые команды":
            ...
            $ ps_humor += 1
            $ ps_team_unity += 2
            $ ps_record_consequence("Вы с Шутником заранее придумали команды на случай отказа связи.")
...
label ps_trace_d6_break:
    ...
    n "Новичок переписывает старую памятку."
    n "Первую строку «не ошибаться» она зачеркнула."
    n "Вместо неё написала: «остановиться и проверить»."
    ...
    menu:
        "Сказать, что она уже может помогать другим":
            ...
            $ ps_newbie_trust += 2
            $ ps_humanity += 1
            $ ps_record_consequence("Новичок впервые увидела в себе не проблему, а опору для другого человека.")
        ...
        "Попросить добавить памятку в общий чат":
            ...
            p "Шутник будет. Остальные сохранят."
...
label ps_trace_d6_mezzanine:
    ...
    n "За старой панелью лежит чёрный пластиковый конверт."
    n "На нём тот же знак, что был на пломбе."
    n "Внутри — накладная V-13."
    ...
    menu:
        ...
        "Позвать Шутника и сверить знак":
            $ ps_add_route("joker", 2)
            $ ps_team_unity += 1
...
label ps_trace_d6_packing:
    ...
    n "Шутник наклеивает на пустую коробку этикетку «ЧЕСТНЫЙ ОТЧЁТ»."
    ...
    menu:
        "Перечислить вслух всё, что нельзя потерять в отчёте":
            ...
            $ ps_integrity += 1
            $ ps_team_unity += 1
            $ ps_record_consequence("Вы с Шутником собрали человеческую версию отчёта.")
...
label ps_trace_d6_control:
    ...
    n "Супервайзер открывает локальный архив камер."
    n "Запись остановки ещё существует в кэше терминала."
    ...
    menu:
        ...
        "Попросить Супервайзера подписать время выгрузки":
            $ ps_add_route("supervisor", 2)
            $ ps_supervisor_respect += 2
            $ ps_integrity += 2
            $ ps_record_consequence("Супервайзер поставил подпись под копией, которую нельзя назвать слухом.")
...
label ps_trace_d6_dock:
    ...
    n "Ветеран стоит у открытых ворот."
    n "Холодный воздух двигает край бинта на его руке."
    ...
    menu:
        "Пообещать не оставлять его историю без подписи":
            ...
            $ ps_integrity += 1
            $ ps_team_unity += 1
            $ ps_record_consequence("Ты пообещал Ветерану довести историю подъёмника до конца.")
        ...
        "Спросить, почему он всё ещё остаётся":
            ...
            $ ps_humanity += 1
            $ ps_endurance += 1
            $ ps_record_consequence("Ветеран признался, что остаётся из-за людей, а не из-за склада.")
...
label ps_route_climax:
    ...
    if ps_route_scene_id == "newbie":
        ...
        newb "Я переписала объяснительную."
        newb "Не так, как они оставили в шаблоне."

        p "Страшно?"
        newb "Да."
        newb "Но раньше мне было страшно даже сказать, что экран завис."

        n "Она протягивает тебе лист."
        ...
        newb "Ты сказал, что ошибка — это событие, а не человек."
        newb "Сегодня я наконец поверила."

        p "Тогда подпиши своим именем."
        p "Не их версией тебя."

        $ ps_newbie_trust += 3
        $ ps_humanity += 2
        $ ps_team_unity += 1
        $ ps_record_consequence("Новичок подписала собственную правду вместо готового признания.")
        ...
        p "Ты?"
        vet "Не делай такое лицо."
        vet "Опытный человек тоже может однажды поступить разумно."
        ...
        p "И?"
        vet "Ничего."
        vet "Линия пережила. Люди подхватили."
        ...
        $ ps_team_unity += 2
        $ ps_veteran_safe = True
        $ ps_record_consequence("Ветеран выбрал остановиться раньше, чем работа остановила бы его.")
        $ ps_key_choices = ps_key_choices + ["Ветеран позволил команде стать его опорой."]
        ...
        $ ps_humanity += 1
        $ ps_team_unity += 2
        $ ps_record_consequence("Шутник доверил тебе причину, по которой продолжает смеяться.")
        $ ps_key_choices = ps_key_choices + ["Ты услышал серьёзный голос Шутника и не заставил его прятать обратно."]
        ...
        n "Он кладёт короткую версию в измельчитель."
        n "Не включает."
        ...
        $ ps_integrity += 2
        $ ps_team_unity += 1
        $ ps_record_consequence("Супервайзер выбрал полный отчёт и принял ответственность за участок.")
        $ ps_key_choices = ps_key_choices + ["Супервайзер перестал прятать решения за формулировками системы."]
...
label ps_storm_teaser:
    ...
    n "На телефоне появляется сеть без названия."
    ...
    n "Не складской."
    n "Слишком глубокий, будто коридор продолжается намного дальше стены."
    ...
    n "Утро снаружи остаётся прежним."

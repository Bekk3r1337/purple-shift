...
label chapter2_hook:
    ...
    menu:
        "Подтвердить смену сразу":
            $ ps_efficiency += 2
            $ ps_key_choices = ps_key_choices + [_("Ты подтвердил вторую смену без колебаний.")]
        ...
        "Отложить телефон и дать себе ещё десять минут":
            $ ps_endurance += 2
            $ ps_key_choices = ps_key_choices + [_("Ты отвоевал у системы десять минут тишины.")]
        ...
        "Ответить новичку" if ps_newbie_trust >= 2:
            $ ps_humanity += 2
            $ ps_newbie_trust += 1
            $ ps_key_choices = ps_key_choices + [_("Перед второй сменой ты пообещал новичку быть рядом.")]
...
label chapter3_second_shift:
    ...
    call screen ps_day_card(
        2,
        _("Вторая смена"),
        _("Дорогу до склада ты уже запомнил. Всё остальное придётся учить заново.")
    )
...
label chapter3_sorting_start:
    ...
    call screen ps_tsd_alert(
        "E-17",
        _("ЯЧЕЙКА НЕ НАЙДЕНА"),
        _("Товар остаётся в активной задаче. Требуется ручная привязка.")
    )
...
label chapter3_incident:
    ...
    call screen ps_tsd_alert(
        "STOP-04",
        _("КОНТЕЙНЕР ПЕРЕПОЛНЕН"),
        _("Продолжение операции может привести к падению груза.")
    )
...
label chapter3_incident_stop:
    ...
    $ ps_humanity += 3
    $ ps_endurance += 1
    $ ps_key_choices = ps_key_choices + [_("Ты остановил линию из-за опасного контейнера.")]
...
label chapter3_incident_team:
    ...
    $ ps_efficiency += 1
    $ ps_newbie_trust += 1
    $ ps_key_choices = ps_key_choices + [_("Ты собрал людей и разгрузил опасный контейнер.")]
...
label chapter3_incident_norm:
    ...
    $ ps_humanity -= 2
    $ ps_newbie_trust -= 2
    $ ps_key_choices = ps_key_choices + [_("Ты продолжил работу, несмотря на опасный контейнер.")]
...
label chapter3_supervisor_talk:
    ...
    menu:
        "Хочу, чтобы рядом со мной не боялись просить помощи":
            $ ps_humanity += 3
            $ ps_key_choices = ps_key_choices + [_("Ты выбрал быть опорой для людей.")]
        ...
        "Хочу научиться отвечать за участок":
            $ ps_efficiency += 2
            $ ps_supervisor_respect += 2
            $ ps_key_choices = ps_key_choices + [_("Ты решил расти и отвечать за участок.")]
        ...
        "Хочу заметить, если эта работа начнёт меня ломать":
            $ ps_endurance += 3
            $ ps_key_choices = ps_key_choices + [_("Ты пообещал себе не потерять выход.")]
        ...
        "Хочу хотя бы не разучиться шутить":
            $ ps_humor += 3
            $ ps_key_choices = ps_key_choices + [_("Ты выбрал не отдавать смене свой голос.")]
...
label chapter3_shift_end:
    ...
    call screen ps_shift_report(
        _("Итоги второго дня"),
        _("Вторая смена закончена. То, что ты сделал у контейнера, заметили все.")
    )

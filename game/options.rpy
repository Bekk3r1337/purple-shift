...
Версия 1.3.0 «Следы смены» превращает семь дней в исследуемую историю: карта склада, четыре маршрута персонажей, память последствий, архив документов, галерея и скрытый сигнал Фиолетового Шторма.
...
define config.sample_sound = "audio/scan_soft.ogg"

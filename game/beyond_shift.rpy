    ...
    if day == 1:
        ...
        menu:
            "Записать честно: «Я испугался, но вернулся к людям»":
                ...
                p "Испугался. Всё равно сделал шаг. Пока этого хватит."
            ...
            "Закрыть приложение и ничего не оценивать":
                ...
                p "Не всё должно становиться показателем."
        ...
        menu:
            "Забрать копию в архив дела":
                ...
                n "Ты фотографируешь лист и возвращаешь его в лоток. Оригинал исчезнет. Время - уже нет."
            ...
            "Порвать копию, чтобы её не использовали против Леры":
                ...
                n "Бумага рвётся легко. Последствия - нет."
...
label ps2_route_crisis:
    $ ps2_target = ps_route_target()

    if ps2_target == "newbie":
        show newb relief at ps_center
        with dissolve
        newb "Если завтра всё повторится, я не хочу снова ждать, пока кто-то разрешит мне сказать «стоп»."
        menu:
            "Дать ей право остановить участок самой":
                ...
                $ ps2_record_decision(6, "после смены", "lera_voice", "Передать право голоса", "Лера вошла в финальный день не под защитой, а со своим решением.")
                p "Завтра ты не просишь. Видишь риск - останавливаешь. Я подтвержу."
                newb "Нет. Ты не подтверждаешь. Ты встаёшь рядом."
                p "Договорились."

            "Пообещать всегда говорить за неё":
                ...
                $ ps2_record_decision(6, "после смены", "lera_cover", "Остаться её голосом", "Лере стало спокойнее рядом с тобой и страшнее без тебя.")
                p "Я скажу. Тебе не придётся."
                newb "А если завтра тебя рядом не будет?"
        hide newb

    elif ps2_target == "veteran":
        show vet concerned at ps_center
        with dissolve
        vet "Я научил вас терпеть. Похоже, это была худшая часть моего опыта."
        menu:
            "Попросить его научить смену останавливаться":
                ...
                $ ps2_record_decision(6, "после смены", "victor_stop", "Переучить опыт", "Виктор решил передать не способность терпеть, а способность вовремя остановиться.")
                p "Тогда завтра покажи не как выдержать. Покажи, когда достаточно."
                vet "Поздно учиться."
                p "Зато есть кому передать."

            "Сказать, что смена всё равно держится на нём":
                ...
                $ ps2_record_decision(6, "после смены", "victor_pillar", "Оставить его опорой", "Виктор снова согласился быть человеком, который ломается последним.")
                p "Без тебя мы завтра не вывезем."
                vet "Вот поэтому я и не умею уходить вовремя."
        hide vet

    elif ps2_target == "joker":
        show mem serious at ps_center
        with dissolve
        mem "Завтра, если будет страшно, я опять пошучу. Только ты реши заранее: тебе нужен ведущий или человек?"
        menu:
            "Попросить его сначала назвать страх":
                ...
                $ ps2_record_decision(6, "после смены", "max_honest", "Договорить без шутки", "Макс не потерял юмор. Он перестал использовать его как аварийный выход из каждого разговора.")
                p "Сначала скажи, что страшно. Потом можешь спасать зал."
                mem "Ненавижу новые правила. Особенно хорошие."

            "Попросить его удержать настроение команды":
                ...
                $ ps2_record_decision(6, "после смены", "max_show", "Оставить ведущего", "Команда будет смеяться. Макс снова останется за собственным голосом.")
                p "Если начнётся паника, без тебя развалимся."
                mem "Принято. Человек отменяется, ведущий остаётся."
        hide mem

    else:
        show sv stern at ps_center
        with dissolve
        sv "Завтра любое решение будет моим по должности. Но последствия почему-то снова будут общими."
        menu:
            "Потребовать назвать решение своим именем":
                ...
                $ ps2_record_decision(6, "после смены", "artem_name", "Решение от своего имени", "Артём вошёл в финал не функцией системы, а человеком с ответственностью.")
                p "Тогда завтра никаких «нам поручили». Говоришь: «я решил»."
                sv "И если ошибусь?"
                p "Тогда это будет честная ошибка, а не чужой приказ."

            "Пообещать принять удар вместе":
                ...
                $ ps2_record_decision(6, "после смены", "artem_cover", "Разделить ответственность", "Артём получил союзника и ещё одну возможность не произносить решение самостоятельно.")
                p "Подпишемся вдвоём."
                sv "Так будет безопаснее."
                n "Он не уточняет, для кого."
        hide sv
...
label ps2_storm_echo(day):
    ...
    if ps2_stage == 1:
        ...
        n "Лента движется без коробок. На каждом пустом участке лежит карточка с одним из решений недели. Некоторые написаны твоим почерком, хотя ты их не писал."
        n "Последняя карточка пустая. Система словно оставила место для финала."
...
label ps2_final_convergence:
    ...
    n "До запуска остаётся несколько минут. В раздевалке впервые за неделю находятся все четверо. Никто не садится."
    n "У тебя есть вся история: коды, сообщения, чужие паузы и собственные решения. Но правда, сказанная не вовремя, тоже может стать ударом."

...
init 40 python:
    ...
    ps2_documents = [
        (
            "decision_map",
            "Карта семи дней",
            "Карта собирает след решений: кого они защитили, чего стоили и кто их запомнил.",
        ),
        (
            "collective_state",
    ...
    ps2_messages = [
        {
            ...
            "incoming": [
                "Зачем я сюда пришёл? Из-за денег. Пока этого ответа достаточно.",
        ...
        },
        {
            ...
            "sender": "Я",
            "time": "--:--",
            "preview": "Не подписывай пустой отчёт.",
            "incoming": [
                "Не подписывай пустой отчёт.",
                "Ты уже видел, что бывает, когда все ждут чужой команды.",
            ],
            "replies": [
                {
                    "id": "who",
                    ...
                    if ps2_last_map():
                        for entry in ps2_last_map()[-8:]:
                            frame:
                                ...
                                vbox:
                                    spacing 4
                                    $ entry_day_label = ps_runtime_text("День")
                                    $ entry_phase = ps_runtime_text(entry["phase"])
                                    $ entry_title = ps_runtime_text(entry["title"])
                                    text "[entry_day_label] [entry['day']] · [entry_phase] · [entry_title]" color "#e7d7f7" size 22
        ...
        vbox:
            ...
            hbox:
                ...
                vbox:
                    ...
                    text "Она показывает след. Правильного ответа здесь нет." color "#a695b5" size 21
                ...
                vbox:
                    ...
                    for day in range(1, 8):
                        ...
                        frame:
                            ...
                            vbox:
                                spacing 8
                                $ entry_day_heading = ps_runtime_text("ДЕНЬ")
                                text "[entry_day_heading] [day]" color ("#caa2f6" if day_entries else "#625a69") size 25
...
label ps2_pre_shift(day):
    ...
    menu:
        "Следить, не нужна ли кому помощь":
    ...
    if day == 1:
        ...
        menu:
            "Записать честно: «Я испугался, но вернулся к людям»":
                ...
                $ ps2_apply(ps_integrity=1, ps2_resolve=1, ps2_pressure=-1)
                $ ps2_record_decision(day, "после смены", "honest_note", "Оставить честную запись", "Ты сохранил собственную версию произошедшего - вместе со страхом и растерянностью.")
        ...
        menu:
            "Сесть рядом и разобрать момент без оправданий":
                ...
                $ ps2_record_decision(day, "после смены", "talk_lera", "Разобрать момент", "Лера услышала честное признание: ты тоже не был уверен.")
        ...
        menu:
            "Спросить, что он хотел доказать снимком":
                ...
                $ ps2_record_decision(day, "после смены", "ask_max", "Спросить без шутки", "Макс впервые признался: шутки помогают ему заглушить страх, пока тот не заговорил первым.")

...
label ps2_pre_shift(day):
    ...
    if day == 1:
        n "До выхода остаётся несколько минут. Впервые за утро ты думаешь, каким хочешь вернуться после смены."
    elif day == 2:
    ...
    if day == 1:
        ...
        menu:
            ...
            "Потребовать исправить учёт официально":
                ...
                p "Если помощь считается простоем, у вас правило сломано. Его и исправляйте."
        ...
        menu:
            ...
            "Подписать формулировку и потребовать ремонт после смены":
                ...
                n "Одного слова хватает, чтобы компромисс показался честным. Цена от этого меньше не становится."
...
label ps2_extended_epilogue:
    ...
    if ps_final_ending in ("truth", "people", "voice"):
        ...
        with fade
        n "Через неделю на остановке снова стоят четверо. График тут ни при чём: они договорились дождаться друг друга."

...
label chapter4_day_four:
    ...
    menu:
        "Ответить: «Ничего не подписывай без меня»":
            ...
            newb "Хорошо."
            n "Одно слово. На этот раз Лера отвечает собранно, без вчерашней паники."
    ...
    if ps_break_target == "newbie":
        ...
        if ps_fourth_day_path == "оставил виноватой":
            ...
            newb "Я запомню."
            n "До прощения ещё далеко. Но разговор хотя бы начался."
        else:
            newb "Злюсь."
    ...
    if ps_efficiency >= 10:
        ...
        n "Твоё имя двигается вверх и вниз. Даже система, кажется, не может решить, чего ты стоишь."
...
label chapter5_day_five:
    ...
    n "Пятый день встречает дождём. Фиолетовая вывеска расплывается в мокром асфальте, и на секунду отражение утягивает склад глубоко под землю."
    ...
    if ps_newbie_trust >= 4:
        n "Она говорит увереннее, чем в первый день. Голос по-прежнему тихий, зато разрешения существовать она уже не просит."
    else:
        n "После фразы она сразу отходит, по старой привычке ожидая наказания за лишний голос."
...
label chapter6_day_six:
    ...
    n "Ручку положили точно напротив тебя: остаётся повторить готовое решение чернилами."
    ...
    if ps_case_result == 3:
        ...
        n "Три материала. Три независимых времени. Из них складывается цепочка событий, которую уже не спишешь на мнение сотрудника."
...
label chapter7_day_seven:
    ...
    menu:
        ...
        "Открыть рейтинг перед выходом":
            $ ps_efficiency += 1
            $ ps_burnout += 1
            n "Твоё имя в первой части списка. Точное место скрыто до конца смены. Система оставляет интригу и превращает рейтинг в шоу."
    ...
    menu:
        ...
        "Снять жилет и выйти из потока":
            ...
            n "Ты спокойно выходишь из потока, без бегства и хлопанья дверью."
...
label ending_people:
    ...
    n "Лера всё ещё боится, но остаётся: теперь она знает, что рядом кто-то тоже видит происходящее."
...
label ending_voice:
    ...
    n "Конвейер замирает, когда вся смена одновременно решает услышать друг друга."
...
label ending_exit:
    ...
    n "Первые шаги даются тяжело. Возвращаться не хочется — просто семь дней тебя учили не останавливаться."
    ...
    n "Ты смеёшься прямо на улице — впервые за неделю по-настоящему, без необходимости что-то пережить."

    n "Твоё имя исчезает из рейтинга. Через минуту строки сдвигаются и стирают последний след твоего места в списке."

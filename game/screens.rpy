...
style window:
    ...
    background Frame(Solid("#120a1f"), 18, 18)
    xpadding 36
    ypadding 28
...
screen main_menu():
    ...
    add Transform("ps_bg_purple", alpha=0.35)
    add Transform("ps_bg_soft", alpha=0.06) at ps_breathe_bg

    # лёгкое затемнение для глубины
    add Solid("#000000") alpha 0.12

    # виньетка (упрощённая)
    add Transform(Solid("#000000"), alpha=0.10) at ps_vignette_soft
...
screen about():
    ...
    use game_menu(_("Об игре"), scroll="viewport"):
        ...
        vbox:
            ...
            label "[config.name!t]"
            text _("Версия [config.version!t]")  # без лишних \n

            null height 25

            # Короткое “о чем игра” — можно править как хочешь
            text _("Визуальная новелла о смене, которая не заканчивается даже после выхода со склада.")
            text _("Тишина, шум системы и люди, которые учатся выживать в потоке.")

            null height 30

            text _("Движок: Ren’Py [renpy.version_only]")
            text _("Платформа: ПК")

            null height 18

            textbutton _("Лицензии и открытый код") action OpenURL("https://www.renpy.org/doc/html/license.html")
    ...
    use game_menu(_("Панель управления сменой"), scroll="viewport"):
        ...
        vbox:
            ...
            hbox:
                ...
                vbox:
                    style_prefix "check"
                    label _("Автоматизация")
                    textbutton _("Максимальная") action Preference("skip", "toggle")
                    textbutton _("Осторожная") action Preference("after choices", "toggle")
                    textbutton _("Только служебное") action Preference("transitions", "toggle")

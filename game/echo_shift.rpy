...
init 18 python:
    ...
    ps_route_variant_catalog = {
        ...
        "veteran": {
            "growth": {
                ...
                "short": "доверие",
                "description": "Виктор учится включать остановку в опыт, не считая её поражением.",
                "accent": "#ffd786",
    ...
    ps_investigation_events = [
        ...
        },
        {
            ...
            "detail": "Кто-то слышал, что Лера перепутала участок.",
            "document": None,
            "accent": "#8fa5bf",
        },
        {
...
screen ps_storm_mimic():
    ...
    frame:
        ...
        vbox:
            ...
            text "Смотри глубже стиля сообщения: чего каждый человек боится и чему научился за неделю.":
                color "#c1b2cb"
                size 21
                text_align 0.5
                xalign 0.5
        ...
        if ps_turn_route == "newbie":
            ...
            menu:
                ...
                "Предложить дать знак и сразу взять разговор на себя":
                    ...
                    n "Лера кивает слишком быстро. Возможность промолчать оказалась удобнее, чем она ожидала."
            ...
            menu:
                "Попросить сообщить о боли Артёму сейчас":
                    ...
                    $ ps_endurance += 1
                    $ ps_team_unity += 1
                    p "Не надо дотягивать. Скажи Артёму сейчас, пока участок можно закрыть спокойно и своими силами."
                ...
                "Согласиться закончить участок и никому не говорить":
                    ...
                    n "Он улыбается с облегчением: ему снова разрешили работать через боль, и этого оказалось достаточно."
            ...
            menu:
                "Потребовать назвать решения своими именами":
                    $ ps_record_route_tendency("supervisor", "growth", 2, "Артём согласился объяснять решения и больше не прикрывать их должностью.")
                    $ ps_integrity += 1
...
label ps_deep_investigation_scene:
    ...
    n "Факты уже лежат в папке. Морозов пролистает её и уцепится за разрывы между страницами. Ты раскладываешь события по времени."
    ...
    if ps_investigation_result >= 9:
        ...
        $ ps_record_consequence("Ты собрал последовательность решений и показал, как каждое из них приближало аварию.")
...
label ps_storm_mimic_scene:
    ...
    if ps_storm_mimic_correct:
        ...
        $ ps_record_consequence("Подделку V-13 выдала одна деталь: рядом с вами Макс уже научился говорить иначе.")

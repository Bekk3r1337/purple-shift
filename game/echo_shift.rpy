...
label ps_storm_mimic_scene:
    ...
    if ps_storm_mimic_correct:
        ...
        n "Ты нажимаешь на сообщение Макса. Его время меняется первым: 06:13 превращается в --:--."
        n "Текст распадается на отдельные буквы. Под ними появляется новая строка: «ГОЛОС ОПРЕДЕЛЁН ПО ПАМЯТИ СМЕНЫ»."
        ...
        n "Отмеченное сообщение гаснет. Через секунду возвращается с обычным временем и отметкой «прочитано»."
        n "Настоящий автор отвечает коротко: «Это был я». Поддельный голос остаётся в списке."
...
label ps_zero_shift:
    ...
    n "Проходная уже закрыла вашу неделю, но телефон показывает незавершённую операцию. Время начала — 00:00. Время окончания отсутствует."
    n "В карточке нет номера сотрудника. Только строка: «СМЕНА 0 // НЕ ПЕРЕДАНА»."
    ...
    n "На старом табло появляются фамилии сотрудников, которых давно нет в графике. Последняя строка остаётся пустой, пока ты не подходишь ближе."
    n "Система вписывает твоё имя и задаёт первый вопрос без готового ответа: «КТО ПЕРЕДАСТ ЭТУ СМЕНУ?»"
